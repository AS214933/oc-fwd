...
## 5xx / 网络错误快速重试（`ZEN_RETRY_FAST_5XX`）

开启旋转出口（`ZEN_ROTATE_IP=true`，默认）或配置 socks5 时，**每个重试尝试都会新建连接、
拿到一个全新的随机出口 IP**——等待退避并不会让下一次尝试更"安全"。因此：

- 默认（`ZEN_RETRY_FAST_5XX` 跟随 `ZEN_ROTATE_IP`，即默认 true）：上游 **5xx / 网络错误立即重试（0ms）**，
  整个重试阶梯最多耗时 ≈ N 次拨号 + 上游响应时间，而不是 2s+4s+8s 的指数等待；
- `429` 与带 `Retry-After` 的错误仍走原阶梯（限速需要墙钟冷却，换 IP 不解决配额）；
- 关闭旋转出口（连接复用，同一出口）或显式 `ZEN_RETRY_FAST_5XX=false` 时恢复经典指数退避。
